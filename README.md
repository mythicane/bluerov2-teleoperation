...
BlueROV2 (Heavy) running BlueOS. Includes a small research study on
identifying and rejecting environmental (current) disturbances from the
onboard IMU signal.

This repo ships **no recorded episodes** — the `grab <color> rod[ under
currents]/` folders are empty skeletons (`data/`, `meta/`, `videos/`) so you
can either record your own data with `src/controller_teleop.py`, or drop in
the published dataset. The dataset this project was developed against is on
HuggingFace:
...
- BlueROV2 (Heavy config, 6 vectored thrusters) running BlueOS, reachable
  over the Fathom-X tether at `192.168.2.2` (default — change in `config.py`
  if yours differs).
...
- `TLOG_DIR` — where QGroundControl saves `.tlog` files, if you use
  `src/explore.py`.
- `DATA_ROOT` — where task dataset folders live. Defaults to the repo root
  (i.e. next to the `grab <color> rod/` folders already in this repo).
...
under `DATA_ROOT` (e.g. `"grab red rod"`). Left stick = forward/strafe, right
...
```bash
python inspect_parquet.py "grab red rod/data/chunk-000/episode_000000.parquet"
```

## Model inference / DAgger

`deployment_scripts/deploy_bluerov.py` runs a trained ACT policy on the
vehicle with a temporal-ensembling action chunker, plus DAgger-style
human-override data collection (stick input above a deflection threshold
takes over from the policy and is logged with `is_intervention=True`):
...
against a `grab <color> rod[ under currents]` folder (or the HuggingFace
...
This expects the `grab <color> rod[ under currents]` folders to be populated
(see Data format above) — it pools all 4 rod colors per condition. A
